...
export const initSocket = (server) => {
  ...
  io.on('connection', (socket) => {
    ...
    // Handle permission update events from client
    socket.on('permission-updated', (data) => {
      console.log('Permission update event received:', data)
      // Broadcast to all admin users
      socket.to('admin_room').emit('rbac-updated', data)
      // Notify the affected user
      if (data.userId) {
        socket.to(`user_${data.userId}`).emit('permission_update', {
